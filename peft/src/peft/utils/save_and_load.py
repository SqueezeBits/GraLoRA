...
def get_peft_model_state_dict(
    ...
    elif config.peft_type == PeftType.GRALORA:
        to_return = {k: state_dict[k] for k in state_dict if "gralora_" in k} 
    else:
        ...
        for k, v in state_dict.items():
            if parameter_prefix in k:
                ...
                if "." in suffix:
                    ...
                    suffix_replaced = suffix.replace(suffix_to_replace, f"{adapter_name}.{suffix_to_replace}")
                    k = f"{k.split(parameter_prefix)[0]}{parameter_prefix}{suffix_replaced}"
                else:
                    k = f"{k}.{adapter_name}"
