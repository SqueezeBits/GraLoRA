...
class BaseTunerLayer(ABC):
    ...
    def set_base_layer(self, new_base_layer: nn.Module) -> None:
        """
        (Recursively) set the base_layer.

        This is necessary for the case that the tuner layer wraps another tuner layer.

        """
        parent_layer = self
        while hasattr(parent_layer, "base_layer"):
            child_layer = parent_layer.base_layer
            if not hasattr(child_layer, "base_layer"):
                parent_layer.base_layer = new_base_layer
                return
